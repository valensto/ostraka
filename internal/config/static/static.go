...
func BuildWorkflows(bs [][]byte) ([]*workflow.Workflow, error) {
	...
	for _, b := range bs {
		...
		if err != nil {
			return nil, fmt.Errorf("error converting staticWorkflow to workflow: %w", err)
		}
...
func (sw staticWorkflow) toWorkflow() (*workflow.Workflow, error) {
...
func (sc staticCondition) toCondition() (*workflow.Condition, error) {
...
func (se staticEvent) toEvent() (*workflow.Event, error) {
...
func (si staticInput) toInput(event *workflow.Event) (*workflow.Input, error) {
